...
class EbayConnectionController extends Controller
{
    ...
    public function submit_ebay_connection(Request $request) {
        ...
        if(count($ebay_connection_detail) >= 1){
            ...
            //Call Api Perameters
            $api_endpoint = $ebay_connection_detail['api_auth_uri'] ?? "";
            $clientId = $ebay_connection_detail['app_id'] ?? "";
    ...
    public function ebay_authorization_callback(Request $request) {
        ...
        if(count($ebay_connection_detail) >= 1){
            ...
            $clientId = $ebay_connection_detail['app_id'] ?? "";
            $clientSecret = $ebay_connection_detail['client_secret'] ?? "";
            $redirectUri = url('/').'/customer/get-ebay-connection';
            ...
            if($access_token){
                ...
                // Redirect after un successful authentication
                return redirect('customer/my-account')->with('unsuccess', 'Oops Something Wrong With eBay Authentication.');
            }
        } else {
            // Redirect after something wrong
            return redirect('customer/my-account')->with('unsuccess', 'eBay Connection Not Enabled. Please Contact With Support.');
        }
    }
